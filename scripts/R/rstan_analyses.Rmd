...
save(fit, file = "ignore/fit_model0.rda")
```
...
eta_rois %>%
  ...
  geom_errorbar(aes(ymin = eta_unscaled_int_95_lower, ymax = eta_unscaled_int_95_upper), width = 0) +
  geom_point(aes(y = eta_unscaled_median)) +
  geom_point(aes(y = cope_mean), color = "hotpink") +
  labs(x = "ROI number (arbitrary)",
       y = "COPE/beta value (arbitrary units)",
       title = "Posterior predictive check: ROI-specific etas don't look great",
