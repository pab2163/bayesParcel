...
generated quantities {
  ...
  vector [N_subj] rep_theta_subj; // Create subject replicates. 
  // Doesn't make sence to create ROI replicates, as sadly, there is no other brain.
  
  for (i in 1:N)
    rep_cope[i] = normal_rng(alpha + tau_roi * eta_roi[roi][i] + tau_subj * eta_subj[subj][i], varcope[i]);
    
  for (i in 1:N_subj)
    rep_theta_subj[i] = alpha + tau_subj * normal_rng(0, 1);
}
```
